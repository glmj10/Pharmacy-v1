...
export const apiUtils = {
    ...
    fromAxiosError(axiosError) {
        ...
        console.error('API Error:', {
            ...
            detailsType: typeof axiosError.response?.data?.details,
            detailsValue: axiosError.response?.data?.details,
            config: {
        ...
        if (axiosError.response?.data) {
            ...
            if (errorData.details && Array.isArray(errorData.details)) {
                details = errorData.details.map(d => new ValidationError(d.field, d.message));
            } else if (errorData.details && typeof errorData.details === 'object') {
                // Handle case where details is an object, not an array
                details = Object.entries(errorData.details).map(([field, message]) => 
                    new ValidationError(field, Array.isArray(message) ? message.join(', ') : message)
                );
            }
