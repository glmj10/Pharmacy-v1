    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        if (params) {
          ...
          if (params.priceFrom) result = result.filter(p => p.priceNew >= params.priceFrom!);
          if (params.priceTo) result = result.filter(p => p.priceNew <= params.priceTo!);

          if (params.isAscending !== undefined) {
            result.sort((a, b) => params.isAscending ? a.priceNew - b.priceNew : b.priceNew - a.priceNew);
          }
        }
