...
const ProductDetail = () => {
  ...
  const isInWishlist = useMemo(() => {
    ...
    return product?.id ? wishlistItems.includes(product.id) : false;
  }, [product?.id, product?.inWishlist, wishlistItems]);
  ...
  const handleProductWishlistToggle = async () => {
    ...
    const currentInWishlist = product.inWishlist !== undefined ? product.inWishlist : wishlistItems.includes(product.id);
            ...
            <div className="product-actions">
              ...
              <div className="action-buttons">
                ...
                <button
                  ...
                  onClick={handleProductWishlistToggle}
                  disabled={!product || !product.id || loading}
                >
                  <i className={`fas fa-heart ${isInWishlist ? 'filled' : ''}`}></i>
                  {loading ? 'Đang tải...' : (isInWishlist ? 'Đã yêu thích' : 'Yêu thích')}
                </button>
              </div>
            </div>
                  ...
                  {relatedProducts.slice(relatedIndex, relatedIndex + relatedVisible).map(relatedProduct => (
                    <div key={relatedProduct.id} style={{ minWidth: 220, maxWidth: 220, flex: '0 0 220px' }}>
                      <ProductCard
                        ...
                        onWishlistToggle={() => handleWishlistToggle(relatedProduct)}
