...
const CategoryList = () => {
  ...
  const isFormValid = () => {
    const basicValid = formData.name.trim() !== '' && formData.type && (typeof formData.type === 'string' ? formData.type.trim() !== '' : true)
  ...
  const openEditModal = (category) => {
    setFormData({
      ...
      type: (category.type && typeof category.type === 'object') ? category.type.code : (category.type || 'PRODUCT'),
    })
    setFormErrors({
      name: '',
      type: '',
  ...
  const getTypeColor = (type) => {
    // Handle both object and string type formats
    let typeValue = type;
    if (typeof type === 'object' && type !== null) {
      typeValue = type.name || type.code || 'UNKNOWN';
    }
